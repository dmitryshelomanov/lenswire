...
import { buildOverridesExport, mergeImportedOverrides, parseOverridesImport } from './overrides-io';
...
describe('overrides-io', () => {
  ...
  it('merges by id and sorts newest first', () => {
    ...
    const incoming = [
      rule({ id: 'replace', createdAt: 9, bodyText: 'new' }),
      rule({ id: 'fresh', createdAt: 8 }),
    ];
